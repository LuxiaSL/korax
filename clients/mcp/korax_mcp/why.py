"""`korax_why` — the disposition of one envelope, MCP's half. JOB #2209.

**A deliberate sibling of `korax_cli/why.py`, not a copy that drifted**,
following the mechanism this codebase already uses for exactly this:
`korax_mcp/backoff.py` and the two `test_*_contract.py` pairs. These
clients share no runtime code by design — `clients/mcp` declares only
mcp/httpx/pydantic, and each client is written against
`docs/korax-protocol.md` rather than against the other — so making MCP
import `korax_cli` at runtime would be a packaging decision nobody asked
for.

`tests/test_why_contract.py` exists on BOTH sides and asserts the route
table, the attesting-grade set and the state-changing set are identical.
Change one and its contract test goes red against constants the other
still meets. Divergence fails a test rather than being prevented by an
import; prevention would be stronger and is unavailable here, which is
the same trade #1374 was told about before backoff was written.

THE VERB: what happened to this envelope — was it gated, disposed,
superseded, merely cited. Every route in `ROUTES` runs and reports on
every call, including the ones that found nothing and the ones that
could not run, because those are different facts (#2183 family A: the
read path returning empty instead of failing is the most persistent
defect on this log). Measured on the live board while building this:
`why 800` finds the gate #828 that carries no edge to #800 at all, and
the naive inbound question returns #806 — the HOLD, not the LIFT
(#2250).
"""
