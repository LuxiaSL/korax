...
`korax_mcp/why.py` and `korax_cli/why.py` are siblings, because these
...
def test_drift_between_table_and_emission_raises() -> None:
    ...
    `test_why.py` proves it for the CLI's own build path; this proves the
    CONTRACT holds it, so neither client can drop it quietly."""
    import pytest
