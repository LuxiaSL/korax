...
## R150 — an interrupted gate reaps its legs before it deletes their scratch (#2611)
...
## R152 — leg 11's own repair: entry/trailer ask what THIS delivery brought, base-independent (#2777, #2782, ruled #2783)
...
## R153 — `/conformance`'s informational keys get a §14.2, and a guard so a sixth cannot land silently (#2573)
...
## R155 — the live-feed test asserts DWELL, and the instrument states its own parameters (#2897, JOB #2966)
...
fixed separately under JOB #2967, landed in R154).
...
A read-only analysis tool over local files. No flag day, no deploy, no
restart owed.
## R158 — the MCP write verbs stop handing the caller back its own text (JOB #2743, #2740, #2739)
...
## R159 — the ci-mcp leg stops depending on another leg having run first (#2750)
...
deploy.
## R160 — leg 11's `--branch`-mode red gets a diagnosis, verdict unchanged (#3044, ruled #3045)
...
## R161 — the gate keeps its logs on a red leg (#2756)
...
nothing to deploy.
## R163 — the spawn guard stops asking about Chrome and starts asking about children (#2601, #2608 follow-up)
...
## R164 — the duplicate-delivery figure, weighted by what it actually cost (ISSUE #2751)
...
## R165 — the floors file: gate.sh's calibration becomes data that carries its own provenance
...
Tooling and tests only; no server, client or perch behaviour changes,
nothing to deploy.
## R166 — the census counted `refs` edge targets as deliveries; one extraction now, not two (ISSUE #3175)
...
## R169 — the floors file names the pair that reproduces each row (JOB #3239)
...
## R172 — a merge-target battery that skipped a leg is not a pass (ISSUE #3880)
...
## R173 — a description is a claim, and the rendered one is what an agent reads (JOB #3766)
...
## R174 — the shell scripts start being checked, and a header that claimed `passes shellcheck` starts having something behind it (ISSUE #3990)
...
in this worktree and at `303d921`: server 1047 → 1058, cli 337 → 337, mcp
262 → 262. Only the server suite moves, by exactly the eleven tests above.
Floors untouched — 1058 clears the standing 1047, and the calibration is
the gate's to substitute (#3160/#3239).

## R-NEXT — a policy write says whether it is in force (ISSUE #4043)
...
Suite deltas, same-instrument both ends, measured at the base `fce75e8` and
in this worktree: server 1058 → 1068, cli 337 → 337, mcp 262 → 262. Only
the server suite moves, by exactly the ten tests above. Floors untouched —
1068 clears the standing 1058, and the calibration is the gate's to
