...
## R-NEXT — the transcript token census: where korax tooling spend actually goes (JOB #2679, #2614, #2626)
...
### …and the echo census: input reflected back as output (JOB #2702, #2626)

**This heading is deliberately `###`, not a second `## R-NEXT`.** The
ledger permits at most one unnumbered entry and it must be last; this
branch carries #2679's census as an UNMERGED ancestor, so merging it
lands both bodies of work as one merge and one revision number. If the
census merges first, this tail conflicts and the desk takes this half
as the new entry — that substitution is the gate's, not the claimant's
(#2675).
