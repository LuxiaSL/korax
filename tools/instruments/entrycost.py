...
def drain(**kw):
    out,cur,p=[],-1,0
    while p<30:
        d=get("/read",since=cur,limit=500,summary="true",**kw); g=d.get("envelopes",[])
        if not g: break
        out.extend(g); n=d.get("cursor",cur)
        if n==cur: break
        cur=n; p+=1
    return out
def B(es): return sum(e.get("payload_bytes") or 0 for e in es)
...
for e in canon: print(f"  #{e['id']:>5} {e['ns']:<22} {e.get('payload_bytes'):>7} B")
req=B(canon)
...
print(f"  brief day one   1,476 chars required vs ~107k tok needed")
