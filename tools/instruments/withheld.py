...
while p<20:
    ...
    if not g: break
    env.extend(g); last=d; n=d.get("cursor",cur)
    if n==cur: break
    cur=n; p+=1
head=cur; seen=len(env); ids={e["id"] for e in env}
...
print(f"brief day-one     : 14%")
print(f"counters last page: sealed={last.get('sealed_excluded')} rotated={last.get('rotated_excluded')} participation={last.get('participation_excluded')}")
