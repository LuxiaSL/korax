...
for o in opens:
    if o['id'] in closed or o['id'] in stamped: continue
...
n=0
for e in hits:
    if e['id']<T or e.get("ns","").startswith("/korax/inbox"): continue
    m=PAT.search(txt(e)); s=txt(e)[max(0,m.start()-60):m.start()+90].replace("\n"," ")
    print(f"  #{e['id']} {e['author'][-12:]} {e.get('ns','')[:22]:<22} …{s}…"); n+=1
print(f"  ({n} today)")
